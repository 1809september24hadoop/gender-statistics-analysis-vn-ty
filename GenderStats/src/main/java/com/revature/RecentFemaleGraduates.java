...
public class RecentFemaleGraduates {
	public static void main(String[] args) throws Exception {
		...
		Job job = new Job();
		job.setJarByClass(RecentFemaleGraduates.class);
		job.setJobName("Countries with less than 30% female graduates!");
		
		FileInputFormat.setInputPaths(job, new Path(args[0]));
		FileOutputFormat.setOutputPath(job, new Path(args[1]));
		
		job.setMapperClass(RecentFemaleGraduateMapper.class);
		job.setReducerClass(RecentFemaleGraduateReducer.class);
